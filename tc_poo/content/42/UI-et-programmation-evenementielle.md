...
<note>
Le but de cette séance est montrer la méthode de progrtammation évènementielle, utilisée lorque l'on construit une UI.
</note>
...
Voir le corrigé de la première séance pour les namespaces. Ici on montre que :
  ...
  - l'ordre d'évaluation des namespace permet de faire de créer des fonctions à partir de fonctions
...
La prmeière fonction teste que le retour est bien une fonction et que 0 est un élément neutre.
...
Question piège. Qu'est sencé rendre `ajoute("truc")("bidule")` ? Si on est dans le mode de pensé python, on utilise la concaténation de chaine et donc c'est sencé rendre "trucbidule". Et c'est le cas avec la proposition de code suivant (attention, pour les chaînes de caractères, `+` n'est pas commutatif...): 
...
La fenêtre ci-dessus est un exemple possible. D'autres design sont bien sur acceptables, mais on doit pouvoir tout justifier. Ici :

  - tout doit avoir un nom (boutons et fenêtre ici)
  - on a mis le bouton à droite pour que le rapport avec let exte soit clair 
...
Le modèle [MVC](https://fr.wikipedia.org/wiki/Modèle-vue-contrôleur) n'est qu'une possiblité pour créer des interfaces de façon *propre* (c'est à dire facilement modifiable). Le principe directeur de toute méthdoe est de pouvoir retrouver facilement ses petits et donc de séparer au maximul les rôles de chaque partie. POur le modèle MVC on a 3 parties : l'affichage (Vue), les données (Modèle) et les liens entre les 2 (Controlleur).
...
Note sur les UML : 
  ...
  - on a pas mis de type pour le bouton. Le type en uml est facultatif.
  - on a pas mis de nom de variable pour les méthodes
...
Le modèle MVC permet de créer des classes, mais dans l'application elles ne sont pas forcément nécessaire : 

  - pour le modèle, il n'y a que des attribut auxquels on accède. Le fait qu'ils soient privé n'est vraiment pas indspensable. Du coup, pour ne pas réinventer la roue, on utilise un dictionnaire pour cette classe. 
  - pour le controlleur il n'y a qu'une méthode. On le remplace par une unique fonction
  
Le modèle UML n'est qu'une aide pour le décopage du code. Le suivre aveuglément ajoute de la complexité inutilement :
  
  - Vue : l'objet de nom `app` et de classe `gui`. 
  - Modèle : le dictionnaire de nom `model`
  - Controlleur : la fonction `press`
...
Pour le code : 
  ...
  - la vue, On crée des objet graphique de nom `"valeur"` (un label) et `"plus_un"` (un bouton). Les getter et setter sont accessible via ces noms. On crée les objet (`addLabel` par exemple), puis on les manipule (`setLabel`). Les nombre à la fin de la création correspondent à ligne et colonne de la fenêtre.
  - on peut utiliser dans la fonction press des choses qui ne sont pas encore définies puisque le code ne sera lue que lorsque l'on pressera le bouton. A cette étpe là tout sera défini.
  - l'action du controlleur est passé à la vue en donnant la fonction utilisée pour le clique : lorsque l'on clique sur l'entité de la vue appelée "plus_un" on exécute la fonction `press` avec comme paramètre ce sur quoi on a cliqué (ie. le bouton). On a pas beosin de ce paramètre ici.
  - `app.go()` crée la fenêtre. Tout e code en-dessous n'est donc pas lu tant que la fenêtre est active. Il faut donc que tout le controlleur soit défini avant cette ligne.
 

{{< note warning>}}
Dans le controlleur on utilise une structure du type `value = value + 1`. On crée donc un nouveau nom `value` dans le namespace concerné qui est égal à la valeur de l'ancien nom `value` + 1.

On ne peut donc pas utiliser uniquement comme Modèle une variable nommée `value` dans le namespace global car si on peut lire un nom (donc `value`) on ne peut créer de nom que dans son propre namespace, ici celui de la fonction (à moins de dire que `value` est de type `global` mais il est déconseillé de faire ce genre de chose. Les namespace existent pour une raison et passer outre va _forcement_ finir en catastrophe).
...
{{<note>}}
	En créant des interfaces qui interagissent avec un utilisateur, il faut toujours avoir à l'esprit la règle de Murphy qui stipule que s'il peut y avoir ds erreurs, il y en aura. Il faut donc toujours faire en sorte que l'erreur ne soit pas possible ou tout du moins vérifier ce qui est écrit par un utilisateur. 
{{</note>}}	
